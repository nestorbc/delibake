...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['imagen'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], $rutaDestinoRelativa)) {
            ...
            $stmt = $conn->prepare("INSERT INTO USERSS (nom, email, cognom, message, rating, imagen_path) VALUES (?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
            ...
            if ($stmt->execute()) {
                ...
                if (strpos($rutaDestinoRelativa, 'Reviews/Reviews/') === 0) {
                    $rutaDestinoRelativa = substr($rutaDestinoRelativa, strlen('Reviews/Reviews/'));
                }
